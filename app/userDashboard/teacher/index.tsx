...
import notifyDrawback from "../helper/notifyDrawback";
...
const Page = () => {
    ...
    const isTeacherInBigGroup = (chronogram: any) => {
        return chronogram.bigGroup.teacher._id == user._id;
    }

    const isTeacherInSmallGroup = (chronogram: any) => {
        return chronogram.smallGroup.teacher._id == user._id;
    }

    const filterChronogramByUserId = (chronogram: any) => {
        return chronogram.filter((item: any) => {
            return item.bigGroup.teacher._id == user._id || item.smallGroup.teacher._id == user._id;
        });
    }

    const filterChronogramByNotTeacherDrawback = (chronogram: any) => {
        return chronogram.filter((item: any) => {

            if (isTeacherInBigGroup(item)) {
                return !item.bigGroup.teacherDrawback;
            }

            if (isTeacherInSmallGroup(item)) {
                return !item.smallGroup.teacherDrawback;
            }
        });
    }
    ...
    const isDescription = async () => {
        ...
        const isDescriptionNotLoaded = chronogram.find((item: any) => {
            return ((item.bigGroup.teacher._id == userData._id && item.bigGroup.scheduleDescription == undefined && !item.bigGroup.teacherDrawback) || (item.smallGroup.teacher._id == userData._id && item.smallGroup.scheduleDescription == undefined && !item.smallGroup.teacherDrawback));
        });
        ...
        <>
            ...
            >
                    ...
                    <View style={{ backgroundColor: 'white', padding: 20, borderRadius: 10 }}>
                        {(scheduleDescription && isTeacherInBigGroup(scheduleDescription) && scheduleDescription.bigGroup.scheduleDescription) && <ScrollView style={{ height: '95%' }}><Text>{scheduleDescription.bigGroup.scheduleDescription}</Text></ScrollView>}
                        {(scheduleDescription && isTeacherInSmallGroup(scheduleDescription) && scheduleDescription.smallGroup.scheduleDescription) && <ScrollView style={{ height: '95%' }}><Text>{scheduleDescription.smallGroup.scheduleDescription}</Text></ScrollView>}
            ...
            <View>
                {!loadingChronogram && <FlatList
                    data={filterChronogramByNotTeacherDrawback(filterChronogramByUserId(chronogram))}
                    ListEmptyComponent={() => (
                    ...
                    renderItem={({ item }: { item: any }) => {
                        return (
                            <View style={styles.item}>
                                ...
                                <Text style={styles.teacher}>Maestro: {item.bigGroup.teacher.gender == 'male' ? 'Hno.' : 'Hna.'} {item.bigGroup.teacher.name} {item.bigGroup.teacher.lastName}</Text>
                                ...
                                {isTeacherInBigGroup(item) && item.bigGroup.scheduleDescription && <Text style={{ color: 'green' }}>PLANIFICACION CARGADA</Text> || <Text style={{ color: 'red' }}>PLANIFICACION PENDIENTE</Text>}
                                ...
                                <Text style={styles.teacher}>Maestro: {item.smallGroup.teacher.gender == 'male' ? 'Hno.' : 'Hna.'} {item.smallGroup.teacher.name} {item.smallGroup.teacher.lastName}</Text>
                                ...
                                <View style={{ marginVertical: 5 }} />
                                {isTeacherInSmallGroup(item) && (item.smallGroup.scheduleDescription && <Text style={{ color: 'green' }}>PLANIFICACION CARGADA</Text> || <Text style={{ color: 'red' }}>PLANIFICACION PENDIENTE</Text>)}
                                ...
                                {((isTeacherInBigGroup(item) && !item.bigGroup.scheduleDescription) || (isTeacherInSmallGroup(item) && !item.smallGroup.scheduleDescription)) && <Button onPress={() => router.push({
                                    pathname: './scheduleEditor', params: {
                                        chronogram: JSON.stringify(item)
                                    }
                                }, { relativeToDirectory: true })}>Cargar planificacion</Button>}

                                {((isTeacherInBigGroup(item) && item.bigGroup.scheduleDescription) || (isTeacherInSmallGroup(item) && !item.smallGroup.scheduleDescription)) && <Button onPress={() => setScheduleDescription(item)}>Ver planificacion</Button>}
                            </View>
                        )
                    }}
                /> || <ActivityIndicator />}
